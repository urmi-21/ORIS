...
public class CummMKskew extends Thread {
    ...
    public void run() {
        ...
        if (saveflag == 1) {
            ...
            try {
                writer.write("Data for the Cumulative MKskew plot ");
                writer.newLine();
                writer.write("Winsize = " + String.valueOf(winsize));
                writer.write("\tIncreament  = " + String.valueOf(increament));
                writer.newLine();
                writer.write("Window Number\t Cumulative[(M - K) / (M + K)] ");
                writer.newLine();
            } catch (IOException ex) {
                Logger.getLogger(Searchseq.class.getName()).log(Level.SEVERE, null, ex);
            }
        ...
        for (start = 0; start + winsize <= sequence.length; start = start + increament) {
            ...
            if (rindex == 0) {
                ...
                cgcresults[rindex++] = sob2.MKskew(subsequence);
            } else {
                xaxis[rindex] = rindex + 1;
                cgcresults[rindex] = cgcresults[rindex - 1] + sob2.MKskew(subsequence);
                rindex++;
            }
        ...
        newplot.doplot(sequence,xaxis, cgcresults, "CumulativeMKskew results", "Window Number", "Cumulative MKskew", winsize, increament);
