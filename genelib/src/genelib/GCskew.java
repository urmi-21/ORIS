...
public class GCskew extends Thread {
    ...
    public void run() {
        ...
        for (start = 0; start + winsize <= sequence.length; start = start +increament) {
            ...
            gcresults[rindex++] = sob2.GCskew(subsequence);
            //update progress bar
            if(rindex%5000==0){
        ...
        newplot.doplot(sequence,xaxis, gcresults, "GCskew results", "Window Number", "(C - G) / (C + G)",winsize,increament);
