...
public class RYskew extends Thread {
    ...
    public void run() {
        ...
        for (start = 0; start + winsize <= sequence.length; start = start +increament) {
            ...
            gcresults[rindex++] = sob2.RYskew(subsequence);
            //update progress bar
            if(rindex%5000==0){
        ...
        newplot.doplot(sequence,xaxis, gcresults, "RYskew results", "Window Number", "(Y - R) / (R + Y)",winsize,increament);
