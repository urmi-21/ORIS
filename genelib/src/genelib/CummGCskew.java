...
/**
 *Class to compute cumulative  GCskew
 * @author urmi
 */
public class CummGCskew extends Thread {
     ...
     */
    public CummGCskew(char[] val, int ws, int inc, int save, String fname) {
        sequence = val;
        winsize = ws;
        increament = inc;
        if (save == 1) {
    ...
    public void run() {
        ...
        if (saveflag == 1) {
            ...
            try {
                writer.write("Data for the Cumulative GCskew plot ");
                writer.newLine();
                writer.write("Winsize = " + String.valueOf(winsize));
                writer.write("\tIncreament  = " + String.valueOf(increament));
                writer.newLine();
                writer.write("Window Number\t Cumulative[(G - C) / (G + C)] ");
                writer.newLine();
            } catch (IOException ex) {
                Logger.getLogger(Searchseq.class.getName()).log(Level.SEVERE, null, ex);
            }
        ...
        for (start = 0; start + winsize <= sequence.length; start = start + increament) {
            ...
            if (rindex == 0) {
                ...
                cgcresults[rindex++] = sob2.GCskew(subsequence);
            } else {
                xaxis[rindex] = rindex + 1;
                cgcresults[rindex] = cgcresults[rindex - 1] + sob2.GCskew(subsequence);
                rindex++;
            }
        ...
        newplot.doplot(sequence,xaxis, cgcresults, "CumulativeGCskew results", "Window Number", "Cumulative GCskew", winsize, increament);
